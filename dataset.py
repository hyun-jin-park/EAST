...
def move_points(vertices, index1, index2, r, coef):
    ...
    Output:
    vertices: vertices where one edge has been shrinked
    """
...
def rotate_img(img, vertices, angle_range=10):
    ...
    for i, vertex in enumerate(vertices):
        new_vertices[i, :] = rotate_vertices(vertex, -angle / 180 * math.pi, np.array([[center_x], [center_y]]))
    return img, new_vertices
...
def get_score_geo(img, vertices, labels, scale, length):
    ...
    for i, vertex in enumerate(vertices):
        if labels[i] == 0:
            ignored_polys.append(np.around(scale * vertex.reshape((4, 2))).astype(np.int32))
            continue

        poly = np.around(scale * shrink_poly(vertex).reshape((4, 2))).astype(np.int32)  # scaled & shrinked
        polys.append(poly)
...
class ICDARDataSet(data.Dataset):
    def __init__(self, data_path, mode='train', scale=0.25, length=512, width=1280, heigth=736):
        ...
        self.mode = mode
        if mode == 'train':
            self.transform = transforms.Compose([transforms.ColorJitter(0.5, 0.5, 0.5, 0.25),
                                                 transforms.ToTensor(),
    ...
    def get_gt_for_eval(self, index):
        file_path = self.gt_files[index]
        points_array = []
        transcription_array = []
    ...
    def get_gt_file_name(self, index):
        return self.gt_files[index]

    def get_img_file_name(self, index):
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            ...
            return self.transform(img), index, width_ratio, height_ratio
